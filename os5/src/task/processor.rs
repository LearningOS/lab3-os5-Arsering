...
impl Processor {
    ...
    fn set_priority_for_current_task(&mut self, prio: usize) {
        self.current.as_mut().unwrap().inner_exclusive_access().schedule.prio = prio;
        self.current
            .as_mut()
            .unwrap()
            .inner_exclusive_access()
            .schedule.stride = BIG_STRIDE / prio;
    }
    ...
    fn update_pass(&mut self) {
        ...
        self.current.as_mut().unwrap().inner_exclusive_access().schedule.pass += stride;

        let pass = self.current.as_ref().unwrap().inner_exclusive_access().schedule.pass;
        if pass + stride < pass {
            self.current.as_mut().unwrap().inner_exclusive_access().schedule.pass = usize::MAX;
        }
    }
    ...
    fn mmap(&mut self, start: usize, len: usize, port: usize) -> isize {
        ...
        let memory_set = &mut self.current.as_mut().unwrap().inner_exclusive_access().memory_set;
        memory_set.mmap(start, len, port)
    }
...
pub fn run_tasks() {
    loop {
        ...
        if let Some(task) = stride_scheduling_task() {
            ...
            // update the pass of current task
            update_pass();
